...
avg_outlet_prop <- mean(df_nyt12_by_outlet$share_of_not_news, na.rm = T)

## Dot plot
df_nyt12_by_outlet$news_source  <- droplevels(factor(df_nyt12_by_outlet$news_source))
df_nyt12_by_outlet$news_source = factor(df_nyt12_by_outlet$news_source, levels = df_nyt12_by_outlet$news_source[order(df_nyt12_by_outlet$share_of_not_news)])

ggplot(df_nyt12_by_outlet , aes(share_of_not_news, news_source, order = share_of_not_news,)) +
...
ggsave("~/Dropbox/Research/Repos/not_news/not_news_us_dev/figs/df_nyt12_dotplot.pdf")


ggplot(df_nyt12_by_outlet, aes(share_of_not_news)) +
  ...
  geom_vline(xintercept = mean(df_nyt12_by_outlet$share_of_not_news, na.rm = T), col = "#cc0000", alpha = .75, linetype = "dotted") + 
...
ggsave("~/Dropbox/Research/Repos/not_news/not_news_us_dev/figs/df_nyt12_density_outlet.pdf")



## Density Plot
ggplot(df_nyt12, aes(pred_soft)) +
  geom_density(aes(y = ..scaled..), color = "#42C4C7", alpha = 0.35) +
  annotate("text", label = paste("Mean =", nolead0s(round(mean(df_nyt12$pred_soft), 2))), x = .54, y = 1, size = 3, colour = "black") + 
  geom_vline(xintercept = mean(df_nyt12$pred_soft, na.rm = T), col = "#cc0000", alpha = .75, linetype = "dotted") + 
...
ggsave("~/Dropbox/Research/Repos/not_news/not_news_us_dev/figs/df_nyt12_density_article.pdf")
